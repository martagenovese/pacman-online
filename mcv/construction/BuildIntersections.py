...
class BuildIntersections(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Building intersections")
    ...
    def on_button_click(self, button):
        ...
        try:
            with open("src/construction/intersections.csv", "a", newline='') as f:
                writer = csv.writer(f, delimiter=';')
                writer.writerow([button.x, button.y])
        except IOError as e:
            messagebox.showerror("Error", f"Failed to write to intersections.csv: {e}")

if __name__ == "__main__":
    if not os.path.exists("src/construction/intersections.csv"):
        with open("src/construction/intersections.csv", "w") as f:
            pass
    app = BuildIntersections()
    app.mainloop()